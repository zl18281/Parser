...
/*"Walker" is used to iterate through the program to find a instruction ahead and an instruction list behind (Tested)*/
...
int is_lt(program *p, symbol_table *t, turtle *turt_pointer)
{
   if((p->head + 1) == p->tail){
      if(strcmp(p->wds[p->head], "LT") == 0 && is_varnum(t, p->wds[p->head + 1])){
         if(turt_pointer->turn_on_off == 1){
            turt_pointer->radius = turt_pointer->radius - (to_num(t, p->wds[p->head + 1]) / 180) * 3.14;
            printf("\nLT\n");
         }
         return 1;
      }
...
/*for loop is inserted to conduct the instruction list nested in DO (Tested)*/
...
{
    if((p->head + 2) == p->tail){
      if(strcmp(p->wds[p->head], "MOV") == 0 && is_varnum(t, p->wds[p->head + 1]) && is_varnum(t, p->wds[p->head + 2])){
         if(turt_pointer->turn_on_off == 1){
