...
/*This function prints the symbol table (Tested)*/
...
/*Search for a symbol in the symbol table and change its value (Tested)*/
...
/*Exactly copy one "program" structure into another (Tested in parser)*/
...
/*Convert a polish expression to its numerical value (Tested)*/
...
/*This function change a string(variable or number) to
numeric value (Tested)*/
...
/*Change a char (digit) to a real numerical digit (Tested)*/
int to_digit(char c)
{
   switch (c) {
      case '0':
...
int is_main(program *p, symbol_table *t,
            SDL_Simplewin *sw_pointer, turtle *turt_pointer)
{
...
int is_instruction_list(program *p, symbol_table *t,
                        SDL_Simplewin *sw_pointer, turtle *turt_pointer)
{
   int walker = p->head;
...
int is_instruction(program *p, symbol_table *t,
                   SDL_Simplewin *sw_pointer, turtle *turt_pointer)
{
   ...
   if((p->head + 1) == p->tail){
      if(strcmp(p->wds[p->head], "FD") == 0 && is_varnum(t, p->wds[p->head + 1])){
         if(turt_pointer->turn_on_off == 1){
            ...
            turt_pointer->turtle_x = temp_turt->turtle_x +
            to_num(t, p->wds[p->head + 1]) * cos(temp_turt->radius);
            turt_pointer->turtle_y = temp_turt->turtle_y +
            to_num(t, p->wds[p->head + 1]) * sin(temp_turt->radius);
...
int is_lt(program *p, symbol_table *t, turtle *turt_pointer)
{
   if((p->head + 1) == p->tail){
      if(strcmp(p->wds[p->head], "LT") == 0 &&
         ...
         if(turt_pointer->turn_on_off == 1){
            turt_pointer->radius = turt_pointer->radius -
            (to_num(t, p->wds[p->head + 1]) / 180) * M_PI;
            printf("\nLT %f\n", to_num(t, p->wds[p->head + 1]));
         }
         return 1;
      }
...
/*numbers from 1 to 7 are position offsets of words after "DO"*/
...
/*Insert the symbol by calling "insert_symbol_table"
function if it's not in the symbol table (Tested)*/
